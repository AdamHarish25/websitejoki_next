          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map((article) => (
              <Link key={article.id} href={`/blog/${article.slug}`} className="block group">
                <div className="border dark:border-gray-700 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 h-full flex flex-col">
                  ...
                  <div className="p-6 flex flex-col flex-grow bg-white dark:bg-gray-800">
                    <h2 className="text-xl font-bold mb-3 group-hover:text-green-600 transition-colors">{article.title}</h2>
