...
import { ArrowLeft, Loader2, Sparkles, ImagePlus, Type, FileText, Globe } from 'lucide-react';
...
export default function CreateArticlePage() {
    ...
    const editorEn = useEditor({
        extensions: [StarterKit],
        content: '<p>Start writing article...</p>',
        immediatelyRender: false,
        editorProps: { attributes: { class: 'prose prose-sm md:prose-base dark:prose-invert max-w-none p-4 min-h-[300px] focus:outline-none' } },
    });
    ...
    useEffect(() => {
        if (!isSlugManuallyEdited) {
            setSlug(title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').replace(/--+/g, '-').replace(/^-+|-+$/g, ''));
        }
    ...
    const handleSmartTranslateAll = async () => {
        ...
        // Translate Content (Experimental for HTML)
        if (editor && editorEn && editor.getHTML() !== '<p></p>') {
            // Note: Sending raw HTML to basic translator might break formatting, but GTX handles tags ok-ish for simple structure.
    ...
    const handleSubmit = async (e) => {
        ...
        if (!title || !slug || !metaDescription || content === '<p></p>' || !coverImage) {
            alert("Semua field Bahasa Indonesia wajib diisi!");
            return;
        }
                ...
                <div className="max-w-6xl mx-auto space-y-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className='space-y-2'>
                            ...
                            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Buat Artikel Baru</h1>
                            <p className="text-gray-500 dark:text-gray-400">Tulis artikel menarik untuk audiens Anda.</p>
                        </div>
                        <div className="flex gap-3">
                            ...
                            >
                                {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : 'Publikasikan Artikel'}
                            </button>
                        </div>
                    </div>
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="Contoh: 5 Cara Meningkatkan SEO Website..."
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            rows={3}
                                            maxLength={160}
                                            placeholder="Deskripsi singkat menarik untuk hasil pencarian Google..."
                        ...
                        <div className="space-y-8 h-fit lg:sticky lg:top-8">
                            ...
                            <SectionCard title="Pengaturan Publikasi" icon={ImagePlus}>
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600 overflow-hidden">
                                            ...
                                            <input
                                                type="text"
                                                value={slug}
                                                onChange={(e) => { setIsSlugManuallyEdited(true); setSlug(e.target.value.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')); }}
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Gambar Sampul</label>
                                        <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer relative">
                                            ...
                                            />
                                            {coverImage ? (
                                                <div className="text-sm text-green-600 font-semibold break-words">
                                                    Selected: {coverImage.name}
                                                </div>
                                            ) : (
                                                <div className="space-y-2 text-gray-500">
                                                    <ImagePlus className="w-8 h-8 mx-auto text-gray-400" />
                                                    <p className="text-xs">Klik untuk upload gambar (Max 2MB)</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
